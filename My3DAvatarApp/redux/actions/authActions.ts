...
import { setCredentials, setError, setLoading, setUserToken } from "@/redux/authSlice";
import { LoginFormValues, LoginResponsePayload } from "@/types/ValidationSchema";
...
export const loginUser = (postData: LoginFormValues) => async (dispatch: Dispatch): Promise<LoginResponsePayload> => {
  ...
  try {
    ...
    if (!recaptchaToken) throw new Error(i18n.t("auth.recaptchaRequired"));
    ...
    if (loginMethod === "email") {
      ...
      await Keychain.setGenericPassword("firebaseToken", idToken, { service: "firebaseToken" });
      ...
      response.message = i18n.t("auth.socialLoginNotImplemented");
    }
...
export const loginWithGoogleFirebase = (idTokenFromExpo: string) => async (dispatch: Dispatch): Promise<LoginResponsePayload> => {
      const auth = getAuth(app); // initialize here locally for RN native
